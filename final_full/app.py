...
def personal():
    if request.method == 'POST':
        print(request.form)
        #fix id during debugging
        try:
            db.execute("INSERT INTO users ( weight, height, gender) VALUES (?, ?, ?)", request.form.get('weight'), request.form.get('height'), request.form.get('gender'))
